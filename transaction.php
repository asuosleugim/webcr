...
if($act!="" && $act=='del'){
	...
	$resdelstock = mysqli_query($sqldelstock);
	
}else if ($act!="" && $act=='edt'){
	$sqledtstock = "select * from temptrans where barcode = '".$_GET['barcode']."' and shopid = '".$_GET['shopid']."'";
	$resedtstock = mysqli_query($sqledtstock);
	$rowedtstock = mysqli_fetch_assoc($resedtstock);
...
if(isset($_POST['save'])){
	$sqltransmaster = "insert into hqtrnmaster (shopid)
							values ('".$loginshopid."')";
	//echo $sqltransmaster;
	$restransmaster = mysqli_query($sqltransmaster);
	$mastertransid = mysqli_insert_id();
	if($restransmaster){
		...
		//echo $sqltrans;
		$restrans = mysqli_query($sqltrans);
		if($restrans){
			$sqldeltemp = "delete from temptrans where shopid = '".$loginshopid."'";
			//echo $sqldeltemp;
			$resdeltemp = mysqli_query($sqldeltemp);
		}
	}
}
...
*/
		...
		$resrec = mysqli_query($sqlrec);
		$nrec = mysqli_num_rows($resrec);
